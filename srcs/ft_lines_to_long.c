...
/*   Updated: 2017/05/18 07:11:12 by sclolus          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* static void		ft_set_longs(char *line, t_board *board, uint32_t i, uint32_t u) */
/* { */
/* 	uint32_t	x; */
/* 	uint32_t	size; */
/* 	char		c; */
...
/* 	x = 0; */
/* 	size = board->size_longs; */
/* 	while (x < sizeof(uint64_t) * 8 && (c = line[(i * sizeof(uint64_t) * 8) + x + 4])) */
/* 	{ */
/* 		if (c == 'O' || c == 'o') */
...
{
	board->player_1 = longs;
	board->player_2 = longs + board->size_longs;
}
		...
		while (i < board->long_nbr)
		{
			...
			while (x < sizeof(uint64_t) * 8 && ((char*)lst->content)[(i * sizeof(uint64_t) * 8) + x + 4])
			{
				if (((char*)lst->content)[(i * sizeof(uint64_t) * 8) + x + 4] == 'O'
					|| ((char*)lst->content)[(i * sizeof(uint64_t) * 8) + x + 4] == 'o')
					longs[u * board->long_nbr + i] |= (1UL << ((sizeof(uint64_t) * 8) - 1)) >> x;
				else if (((char*)lst->content)[(i * sizeof(uint64_t) * 8) + x + 4] == 'x'
					|| ((char*)lst->content)[(i * sizeof(uint64_t) * 8) + x + 4] == 'X')
					longs[u * board->long_nbr + i + size] |= (1UL << ((sizeof(uint64_t) * 8) - 1)) >> x;
				x++;
			}
			i++;
